...
return new class extends Migration
{
    public function up()
    {
        Schema::create('denunciaUsuario', function (Blueprint $table) {
            $table->id();
            $table->foreignId('user_id')->constrained('users')->onDelete('cascade'); // Usuário que fez a denúncia
            $table->foreignId('user_denunciado_id')->constrained('users')->onDelete('cascade'); // ID do post denunciado
    ...
    {
        Schema::dropIfExists('denunciaUsuario');
    }
};
